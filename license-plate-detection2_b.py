...
#S		                writeShapes('%s/%s_lp.txt' % (output_dir,bname),[s])
		                ...
		                if len(R):
							...
							lp_texts.append(null)
		            else:

		                lp_bboxes.append(null) # [ST210921] see Line 201
...
#S		                tempfile_handlers.append(f)
		        ...
		        if Lcar: # [ST210921] Checking if any cars detected

					for i, lcar in enumerate(Lcar):
						# [ST210921] draw box for cars detected
						draw_label(Iorig, lcar, color=YELLOW, thickness=3) 

						if lp_bbox: # [ST210921] Checking if any license plates detected
							pts = lp_bboxes[i][0].pts*lcar.wh().reshape(2,1) + lcar.tl().reshape(2,1)
